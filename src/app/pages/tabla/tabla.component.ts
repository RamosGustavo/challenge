...
export class TablaComponent implements OnInit, OnDestroy, AfterViewInit {
...
export class CustomSpeedSummary {
    ...
    public operate(data?: Athlete[]): IgxSummaryResult[] {
        let speedData: any;
        if(data){
            speedData = data.reduce((acc, val) => acc.concat(val), [] as Athlete[]).map(rec => rec.Speed);
        }
        const result = [];
        result.push(
            {
